...
- ウィジェットは親要素の幅いっぱい（最大1120px・中央寄せ）で描画され、560px以上でタブレット向け、900px以上で右サイド（中村・吹き出し・チェックリスト）付きの2カラムになります。
- LINEボタン（査定の流れ末尾）は `config.js` の `lineUrl` で制御。空にすれば非表示。
...
## デザイン（AIサイボーグ中村版・2026-09-18）

参考画像 `画像/AI中村画面 (1).jpg` とパーツ `画像/nakamura_parts/` に合わせて全面変更。暗い倉庫背景・黄色見出し・ピンクネオン・AIサイボーグ中村。
構成: ヒーロー → タブ（写真で査定 / 重さで査定） → ステップ → パネル → 白い査定結果カード（ピンクの「推定買取価格」にリール） → 特徴4項目 → 査定の流れ（4ステップ + LINEボタン）。
LINEボタンのリンク先は `config.js` の `lineUrl`（空にすると非表示）。寸法・操作の基準はデジタル庁デザインシステムを継続。
